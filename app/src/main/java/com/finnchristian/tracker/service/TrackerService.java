...
import com.finnchristian.tracker.ui.MainActivity;
...
public class TrackerService extends Service {
    ...
    protected Notification createNotification(final int trackId) {
        ...
        // Lookup whether to use two pane layout or not. Value is read from config.xml.
        final boolean twoPaneLayout = getResources().getBoolean(R.bool.twoPaneLayout);

        final Intent intent;

        if(twoPaneLayout) {
            intent = new Intent(getApplicationContext(), MainActivity.class);
            intent.putExtra(MainActivity.ARG_TRACK_ID, track.getId());
        }
        else {
            intent = new Intent(getApplicationContext(), TrackDetailsActivity.class);
            intent.putExtra(TrackDetailsActivity.ARG_TRACK_ID, track.getId());
        }
        ...
        final PendingIntent pendingIntent = stackBuilder.getPendingIntent(0, PendingIntent.FLAG_CANCEL_CURRENT);
